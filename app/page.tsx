...
export default function Home() {
  ...
  const actualizarFila = (id: number, campo: keyof Omit<TableRow, 'id'>, valor: string) => {
  ...
  const calcularPromedio = () => {
    const filasActivas = rows.filter(row => 
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={programaEducativo} onValueChange={setProgramaEducativo}>
                ...
                <div>
                  ...
                  <Select value={cargoAcademico} onValueChange={setCargoAcademico}>
                    <SelectTrigger className="w-full">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-600 text-white">
                      <th className="border border-gray-400 px-4 py-3 text-sm font-medium">N#</th>
                      <th className="border border-gray-400 px-4 py-3 text-sm font-medium">Actividad</th>
                      <th className="border border-gray-400 px-4 py-3 text-sm font-medium">Meta</th>
                      <th className="border border-gray-400 px-4 py-3 text-sm font-medium">Indicador</th>
                      <th className="border border-gray-400 px-4 py-3 text-sm font-medium">Nivel de logro</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((row, index) => (
                      <tr key={row.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        ...
                        <td className="border border-gray-300 p-2">
                          <Textarea
                            value={row.actividad}
                            onChange={(e) => actualizarFila(row.id, 'actividad', e.target.value)}
                        ...
                        <td className="border border-gray-300 p-2">
                          <Textarea
                            value={row.meta}
                            onChange={(e) => actualizarFila(row.id, 'meta', e.target.value)}
                            className="min-h-[60px] resize-none border-none focus:ring-0 focus:border-none shadow-none p-2"
                        ...
                        <td className="border border-gray-300 p-2">
                          <Textarea
                            value={row.indicador}
                            onChange={(e) => actualizarFila(row.id, 'indicador', e.target.value)}
                            className="min-h-[60px] resize-none border-none focus:ring-0 focus:border-none shadow-none p-2"
                        ...
                        <td className="border border-gray-300 p-2">
                          <Textarea
                            ...
                            onChange={(e) => actualizarFila(row.id, 'nivelLogro', e.target.value)}
                            className="min-h-[60px] resize-none border-none focus:ring-0 focus:border-none shadow-none p-2"
              ...
              <div className="flex justify-center">
                <div className="text-center">
                  ...
                  <div className="inline-block p-4 bg-white border border-gray-300 rounded-lg">
                    <QRCodeGenerator text={cargoAcademico} onQRCodeGenerated={setQrCodeDataUrl} />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between items-center pt-4">
              <div className="text-lg font-medium text-gray-900">
                Promedio total: <span className="font-bold">{calcularPromedio()}</span>
              </div>
